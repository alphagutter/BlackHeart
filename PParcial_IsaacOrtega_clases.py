    ...
    def jugar(self, Consola):
            if Consola.disponible == False:
                ...
                if self.estado_animo < 3:
                    ...
                    print(f'El cliente {self.nombre} está jugando a la consola {Consola}')
                    self.entusiasmo += 1
                else:
                    self.entusiasmo += 3
...
class Comedor:
    ...
    def agregar_trabajador(self, persona):
        if type(persona) == Cajero:
            self.caja.append(persona)
        elif type(persona) == Cajero:
            self.caja.append(persona)
